...
socket.on('connect', () => {
	...
	socket.on('DeliveryFinished', () => {
		console.log('DeliveryFinished recebido no simulator!');
	...
	setInterval(() => {
		socket.emit('requestLocation', ORDER_ID);
	}, 30000);

	socket.on('locationResponse', (posicao) => {
		console.log(posicao);
	});
});
...
socket.on('locationUpdated', (data) => {
	console.log(
		`Confirmação do Servidor: Rider ${data.riderId} está em movimento.`
	);
});
